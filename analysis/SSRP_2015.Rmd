...
```{r, results = 'hide', echo=TRUE, message = FALSE, warning = FALSE}
cutoff <- function(vals, avg.water, sd.water){
...
for (name in file.names){
  ...
  i1.var <- var(i1.vals)
  results <- cutoff(i1.vals, avg.water, sd.water)
  i1.num <- results[1] 
  i1.mean <- results[2]  
  i1.sd <- results[3] 
  ...
  i2.var <- var(i2.vals)
  results <- cutoff(i2.vals, avg.water, sd.water)
  i2.num <- results[1] 
  i2.mean <- results[2]  
  i2.sd <- results[3] 
...
bp <- barplot(use.means[,3],
        ...
        main = paste("KBS", levels(m1.24$strain), ": 24 hours", sep=""),
...
arrows(bp, use.means[,3] - use.sem[,3], 
       bp, use.means[,3] + use.sem[,3], 
       lwd = 2, angle = 90, code = 0)
arrows(x0 = bp[1,], y0 = use.means[1,3], y1 = use.means[1,3] - use.sem[1,3], 
...
```{r, results = 'hide', echo=FALSE, message = FALSE, warning = FALSE}
png(filename = "./figures/48hr-ResourcesUsed.png",
     width = 700, height = 500, units = "px", pointsize = 17, bg = "white")
plot.new()
...
bp2 <- barplot(use.means,
        ...
        #create a legend
        legend.text = c("Ancestor","Derived"), 
        args.legend = list(col = c("black","white"), bty = 'n', cex = 1.15, x = "top")
        )
#text(1.5,14,"24 hrs", cex = 2, font = 2)
box(lwd = 3)
...
arrows(bp2, use.means - use.sem, 
       ...
       lwd = 2, angle = 90, code = 0)
arrows(x0 = bp2[1,], y0 = use.means[1,], y1 = use.means[1,] - use.sem[1,], 
       lwd = 2, angle =90, code = 0, col = "white")
...
![48 Hour Resource Use](/figures/48hr-ResourcesUsed.png)

We can also make individual figures

```{r, results = 'hide', echo=FALSE, message = FALSE, warning = FALSE}
for(i in m1.48$strain){
  png(filename = paste("./figures/48hr-ResourcesUsed", i, ".png", sep=""),
      width = 700, height = 500, units = "px", pointsize = 17, bg = "white")
  plot.new()
  par(mar = c(3,5,2,1) + 0.1)
  
  st.means <- use.means[, which(colnames(use.means) == i)]
  st.sem <- use.sem[, which(colnames(use.sem) == i)]

  st.bp <- barplot(st.means,
                   # set the plot parameters
                   ylim = c(0,31),
  ...
  # Let's add some error bars
  arrows(st.bp, st.means - st.sem, 
         st.bp, st.means + st.sem, 
         lwd = 2, angle = 90, code = 0)
  arrows(x0 = st.bp[1], y0 = st.means[1], y1 = st.means[1] - st.sem[1], 
         lwd = 2, angle =90, code = 0, col = "white")
